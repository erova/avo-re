    ...
    <div style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{
          ...
          background: colors.bg.elevated, borderRadius: 8, boxShadow: "0 4px 20px rgba(0,0,0,0.4)",
          ...
          <button onClick={() => { onAction("agent"); setOpen(false); }}
            ...
            <div>
              <div style={{ fontWeight: 500, color: colors.text.primary }}>Create Agent Task</div>
              <div style={{ fontSize: 10, color: colors.text.muted }}>AI will work on this</div>
            </div>
          </button>
          <button onClick={() => { onAction("assign"); setOpen(false); }}
            ...
            <div>
              <div style={{ fontWeight: 500, color: colors.text.primary }}>Assign for Review</div>
              <div style={{ fontSize: 10, color: colors.text.muted }}>Send to someone</div>
            </div>
          </button>
          <button onClick={() => { onAction("delegate"); setOpen(false); }}
            ...
            <div>
              <div style={{ fontWeight: 500, color: colors.text.primary }}>Delegate</div>
              <div style={{ fontSize: 10, color: colors.text.muted }}>Make it someone else's problem</div>
            </div>
          </button>
        </div>
      )}
...
export default function GovernanceDashboardV2DarkPage() {
  ...
  return (
    <div style={{ height: "calc(100vh - 56px)", background: colors.bg.page, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1152, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 10, color: colors.accent.purple, fontWeight: 700, textTransform: "uppercase", letterSpacing: "1.5px", background: "rgba(124,58,237,0.15)", padding: "4px 10px", borderRadius: 4 }}>
                Prototype v2
              </span>
              <span style={{ fontSize: 14, fontWeight: 600, color: colors.text.primary }}>GovernAI Effectiveness Dashboard: Microsoft</span>
              <span style={{ fontSize: 11, color: colors.text.muted }}>— Dark mode</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <Link href="/now/board-governance/dashboard-v2" style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: colors.text.secondary, textDecoration: "none", padding: "4px 10px", background: colors.bg.elevated, borderRadius: 4, border: `1px solid ${colors.border.default}` }}>
                <Sun size={12} /> Light mode
              </Link>
              <Link href="/now/board-governance/dashboard" style={{ fontSize: 11, color: colors.accent.purple, textDecoration: "none" }}>
                ← Detailed version
              </Link>
            </div>
          </div>
      ...
      <div style={{ 
        ...
        {/* HEADER */}
        <header style={{ flexShrink: 0, background: "#1E3A5F", boxShadow: "0 2px 8px rgba(0,0,0,0.3)", position: "relative", zIndex: 10 }}>
        ...
        <main style={{ flex: 1, overflowY: "auto", background: colors.bg.card, minHeight: 0 }}>
          ...
          <section style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gap: 6 }}>
              {URGENT_ITEMS.map((item) => (
                <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", borderRadius: 8, background: colors.bg.elevated, border: `1px solid ${colors.accent.red}40` }}>
          ...
          {/* COUNTDOWN */}
          <section style={{ marginBottom: 20 }}>
            <div style={{ background: colors.accent.blueMuted, borderRadius: 10, padding: 16, border: `1px solid ${colors.accent.blue}30` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <div style={{ textAlign: "center", padding: "6px 16px", background: colors.accent.blueMuted, borderRadius: 6, border: `1px solid ${colors.accent.blue}40` }}>
                ...
                {[4, 3, 2, 1].map((week) => {
                  ...
                  return (
                    <button key={week} onClick={() => setExpandedCountdownWeek(isSelected ? null : week)}
                      style={{ flex: 1, padding: "8px 10px", borderRadius: 6, cursor: "pointer", background: isSelected ? colors.accent.blueMuted : colors.bg.elevated, border: `1px solid ${isSelected ? colors.accent.blue : colors.border.default}40`, textAlign: "center" }}>
              ...
              {expandedCountdownWeek && (
                <div style={{ marginTop: 12, background: colors.bg.elevated, borderRadius: 6, padding: 12, border: `1px solid ${colors.border.default}` }}>
                  <div style={{ display: "grid", gap: 6 }}>
                    {COUNTDOWN_ACTIONS.filter(a => a.weeksOut === expandedCountdownWeek).map((action, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", borderRadius: 4, background: action.status === "done" ? colors.accent.greenMuted : colors.bg.card, border: `1px solid ${action.status === "done" ? colors.accent.green : colors.border.default}40` }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {/* TABS */}
          <div style={{ display: "flex", gap: 0, marginBottom: 0, borderBottom: `1px solid ${colors.border.default}` }}>
            {TABS.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                style={{ padding: "10px 16px", background: activeTab === tab.id ? colors.bg.card : colors.bg.elevated, border: `1px solid ${colors.border.default}`, borderBottom: activeTab === tab.id ? `1px solid ${colors.bg.card}` : `1px solid ${colors.border.default}`, borderRadius: "8px 8px 0 0", marginBottom: -1, marginRight: -1, fontSize: 12, fontWeight: activeTab === tab.id ? 600 : 500, color: activeTab === tab.id ? colors.text.primary : colors.text.muted, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
                {tab.icon}
                {tab.label}
              </button>
            ))}
                  ...
                  <div style={{ padding: 16, border: `1px solid ${colors.border.default}`, borderRadius: 8, background: colors.bg.elevated }}>
                    ...
                    <div style={{ display: "grid", gap: 8 }}>
                      {DOGS_NOT_BARKING.map((d, i) => (
                        <div key={i} style={{ padding: 10, borderRadius: 6, background: d.riskLevel === "high" ? colors.accent.redMuted : colors.accent.yellowMuted, border: `1px solid ${d.riskLevel === "high" ? colors.accent.red : colors.accent.yellow}40` }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  ...
                  <div style={{ padding: 16, border: `1px solid ${colors.border.default}`, borderRadius: 8, background: colors.bg.elevated }}>
                    ...
                    <div style={{ display: "grid", gap: 8 }}>
                      {REGULATORY_RADAR.map((r, i) => (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 10, borderRadius: 6, background: r.status === "not-discussed" ? colors.accent.redMuted : r.status === "in-progress" ? colors.accent.yellowMuted : colors.accent.greenMuted, border: `1px solid ${r.status === "not-discussed" ? colors.accent.red : r.status === "in-progress" ? colors.accent.yellow : colors.accent.green}40` }}>
                          <div style={{ flex: 1 }}>
                        ...
                        return (
                          <div key={qi} style={{ display: "flex", justifyContent: "center" }}>
                            <div style={{ width: 32, height: 32, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600,
                              background: count === 0 ? colors.bg.elevated : topic.trend === "emerging" ? `rgba(34, 197, 94, ${0.15 + intensity * 0.35})` : topic.trend === "declining" ? `rgba(113, 113, 122, ${0.15 + intensity * 0.2})` : `rgba(59, 130, 246, ${0.15 + intensity * 0.3})`,
              ...
              <div>
                ...
                <div style={{ display: "grid", gap: 10 }}>
                  {SENTIMENT_SIGNALS.map((signal, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 16, alignItems: "center", padding: "12px 16px", borderRadius: 8, background: signal.type === "positive" ? colors.accent.greenMuted : colors.accent.redMuted, border: `1px solid ${signal.type === "positive" ? colors.accent.green : colors.accent.red}40` }}>
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                          <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 3, fontWeight: 600, background: signal.type === "positive" ? `${colors.accent.green}30` : `${colors.accent.red}30`, color: signal.type === "positive" ? colors.accent.green : colors.accent.red }}>{signal.type === "positive" ? "POSITIVE" : "RED FLAG"}</span>
                ...
                <div style={{ padding: 16, border: `1px solid ${colors.border.default}`, borderRadius: 8, background: colors.bg.elevated }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
                    {MEETING_CARRYOVER.map((m, i) => {
                      ...
                      return (
                        <div key={i} style={{ padding: 12, borderRadius: 6, textAlign: "center", background: isLast ? (isWorsening ? colors.accent.redMuted : colors.accent.greenMuted) : colors.bg.card, border: `1px solid ${isLast ? (isWorsening ? colors.accent.red : colors.accent.green) : colors.border.default}40` }}>
                    ...
                    {overallScores.map((score, i) => {
                      ...
                      return (
                        <div key={i} style={{ textAlign: "center" }}>
                          ...
                          <div style={{ width: 24, height: score * 0.5, borderRadius: 3, background: isLast ? (isDown ? `linear-gradient(180deg, ${colors.accent.red}80, ${colors.accent.red})` : `linear-gradient(180deg, ${colors.accent.green}80, ${colors.accent.green})`) : colors.bg.elevated }} />
              ...
              <div>
                ...
                <div style={{ display: "grid", gap: 6 }}>
                  {(showAllDirectors ? DIRECTOR_EDUCATION : DIRECTOR_EDUCATION.slice(0, 6)).map((d, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "160px 1fr 70px 70px", alignItems: "center", gap: 12, padding: "8px 10px", borderRadius: 6, background: d.status === "at-risk" ? colors.accent.redMuted : d.status === "behind" ? colors.accent.yellowMuted : colors.bg.elevated, border: `1px solid ${d.status === "at-risk" ? colors.accent.red : d.status === "behind" ? colors.accent.yellow : colors.border.default}40` }}>
                      <div>
      ...
      {showAgentModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
          <div style={{ background: colors.bg.card, borderRadius: 16, padding: 24, width: 480, boxShadow: "0 20px 60px rgba(0,0,0,0.4)", border: `1px solid ${colors.border.default}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ fontSize: 18, fontWeight: 700, color: colors.text.primary, margin: 0, display: "flex", alignItems: "center", gap: 8 }}><Bot size={20} /> Create Agent Task</h3>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: colors.text.muted, marginBottom: 8 }}>What should the agent do?</div>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: colors.text.muted, marginBottom: 8 }}>Agent autonomy</div>
              <div style={{ display: "grid", gap: 8 }}>
                {[{ level: "Draft for review", desc: "Agent creates draft, you approve before action" }, { level: "Execute with notification", desc: "Agent acts, you get notified of results" }, { level: "Fully autonomous", desc: "Agent handles end-to-end, reports on completion" }].map((opt, i) => (
